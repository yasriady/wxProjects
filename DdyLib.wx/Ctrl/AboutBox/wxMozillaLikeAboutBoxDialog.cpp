...
wxMozillaLikeAboutBoxDialog::wxMozillaLikeAboutBoxDialog( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
    ...
    // Ddy: 19-Feb-2015
    //wxImage::AddHandler(new wxPNGHandler); // Ddy: relocated into App.cpp
    SetAppName(wxTheApp->GetAppName());
    SetVersion(wxT("1.0.0 b1"));
    SetCopyright(wxString::Format(wxT("%c %i %s"),
    		(wxChar) 0x00A9, wxDateTime::Now().GetYear(),
    		_("All rights reserved. This program is provided as is without any warranty.")));
	SetCustomBuildInfo(wxString::Format(wxT("%s. %s"),
		wxMozillaLikeAboutBoxDialog::GetBuildInfo(wxMozillaLikeAboutBoxDialog::wxBUILDINFO_LONG).GetData(),
		_("Ddy/Btm")));
	//SetHeaderBitmap(wxGetBitmapFromMemory(header_png, sizeof(header_png)));
	//SetHeaderBitmap( wxBITMAP_PNG_FROM_DATA(header) );
	SetHeaderBitmap( wxBITMAP_PNG_FROM_DATA(IMG_6781) );
	ApplyInfo();
...
////@begin wxMozillaLikeAboutBoxDialog content construction
    ...
    m_ContentPanel = new wxPanel( itemDialog1, ID_ContentPanel, wxDefaultPosition, wxSize(294, 340) /*wxSize(200, 300)*/, wxNO_BORDER|wxTAB_TRAVERSAL );
